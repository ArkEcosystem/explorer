<div class="hidden w-full table-container md:block">
    <table>
        <thead>
            <tr>
                <x-tables.headers.desktop.text name="general.delegates.rank" wire:click="$emit('orderDelegatesBy', '{{ OrderingTypeEnum::RANK }}')" />
                <x-tables.headers.desktop.address name="general.delegates.name" wire:click="$emit('orderDelegatesBy', '{{ OrderingTypeEnum::ADDRESS }}')" />
                <x-tables.headers.desktop.status name="general.delegates.status" wire:click="$emit('orderDelegatesBy', '{{ OrderingTypeEnum::STATUS }}')">
                ...
                @endif
                <x-tables.headers.desktop.number name="general.delegates.productivity" wire:click="$emit('orderDelegatesBy', '{{ OrderingTypeEnum::PRODUCTIVITY }}')">
