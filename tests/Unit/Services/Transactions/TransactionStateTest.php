...
it('should determine if the transaction is sent', function () {
    ...
    expect((new TransactionState($transaction))->isSent('sender'))->toBeTrue();
    expect((new TransactionState($transaction))->isSent('recipient'))->toBeFalse();
});
...
it('should determine if the transaction is received', function () {
    ...
    expect((new TransactionState($transaction))->isReceived('recipient'))->toBeTrue();
    expect((new TransactionState($transaction))->isReceived('sender'))->toBeFalse();
});
